...
class AppController extends Controller{
	...
	function beforeFilter(){
		$config=$this->Config->read(null,1);//Lee la configuraciòn
   ...
   // debug($this->action);
    ...
    $this->Auth->loginRedirect = '/productos/index';
    $this->Auth->logoutRedirect = '/autenticacion';
		if($config["Config"]["autenticacion_id"]){
			eval($config["Autenticacion"]["codigo_configuracion"]);//Ejecuta el codigo de configuracion de la autenticaciòn	
		}
	}
	function getList() {
